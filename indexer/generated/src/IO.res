...
let executeBatch = async (sql, ~inMemoryStore: InMemoryStore.t) => {
  ...
  let setMultiTokenERC20_OwnershipTransferreds = entityDbExecutionComposer(
    _,
    ~entityMod=module(Entities.MultiTokenERC20_OwnershipTransferred),
    ~rows=inMemoryStore.multiTokenERC20_OwnershipTransferred->InMemoryTable.Entity.rows,
  )

  let setMultiTokenERC20_TokenCreateds = entityDbExecutionComposer(
    _,
    ~entityMod=module(Entities.MultiTokenERC20_TokenCreated),
    ~rows=inMemoryStore.multiTokenERC20_TokenCreated->InMemoryTable.Entity.rows,
  )

  let setMultiTokenERC20_TokensBurneds = entityDbExecutionComposer(
    _,
    ~entityMod=module(Entities.MultiTokenERC20_TokensBurned),
    ~rows=inMemoryStore.multiTokenERC20_TokensBurned->InMemoryTable.Entity.rows,
  )

  let setMultiTokenERC20_TokensMinteds = entityDbExecutionComposer(
    _,
    ~entityMod=module(Entities.MultiTokenERC20_TokensMinted),
    ~rows=inMemoryStore.multiTokenERC20_TokensMinted->InMemoryTable.Entity.rows,
  )
  ...
  let res = await sql->Postgres.beginSql(sql => {
    Belt.Array.concat(
      ...
      [
        ...
        setRawEvents,
        setDynamicContracts,
        setMultiTokenERC20_OwnershipTransferreds,
        setMultiTokenERC20_TokenCreateds,
        setMultiTokenERC20_TokensBurneds,
        setMultiTokenERC20_TokensMinteds,
      ],
...
module RollBack = {
  ...
  let rollBack = async (~chainId, ~blockTimestamp, ~blockNumber, ~logIndex) => {
    ...
    reorgData->Belt.Array.forEach(e => {
      ...
      | {previousEntity: Some({entity: MultiTokenERC20_OwnershipTransferred(entity), eventIdentifier}), entityId} =>
        inMemStore.multiTokenERC20_OwnershipTransferred->InMemoryTable.Entity.set(
          Set(entity)->Types.mkEntityUpdate(~eventIdentifier, ~entityId, ~shouldSaveHistory=false),
          ~shouldRollbackOnReorg,
        )
      | {previousEntity: Some({entity: MultiTokenERC20_TokenCreated(entity), eventIdentifier}), entityId} =>
        inMemStore.multiTokenERC20_TokenCreated->InMemoryTable.Entity.set(
          Set(entity)->Types.mkEntityUpdate(~eventIdentifier, ~entityId, ~shouldSaveHistory=false),
          ~shouldRollbackOnReorg,
        )
      | {previousEntity: Some({entity: MultiTokenERC20_TokensBurned(entity), eventIdentifier}), entityId} =>
        inMemStore.multiTokenERC20_TokensBurned->InMemoryTable.Entity.set(
          Set(entity)->Types.mkEntityUpdate(~eventIdentifier, ~entityId, ~shouldSaveHistory=false),
          ~shouldRollbackOnReorg,
        )
      | {previousEntity: Some({entity: MultiTokenERC20_TokensMinted(entity), eventIdentifier}), entityId} =>
        inMemStore.multiTokenERC20_TokensMinted->InMemoryTable.Entity.set(
          Set(entity)->Types.mkEntityUpdate(~eventIdentifier, ~entityId, ~shouldSaveHistory=false),
          ~shouldRollbackOnReorg,
        )
      //Where previousEntity is None, 
      //delete it with the eventIdentifier of the rollback event
      | {previousEntity: None, entityType: MultiTokenERC20_OwnershipTransferred, entityId} =>
        inMemStore.multiTokenERC20_OwnershipTransferred->InMemoryTable.Entity.set(
          Delete->Types.mkEntityUpdate(~eventIdentifier=rollBackEventIdentifier, ~entityId, ~shouldSaveHistory=false),
          ~shouldRollbackOnReorg,
        )
      | {previousEntity: None, entityType: MultiTokenERC20_TokenCreated, entityId} =>
        inMemStore.multiTokenERC20_TokenCreated->InMemoryTable.Entity.set(
          Delete->Types.mkEntityUpdate(~eventIdentifier=rollBackEventIdentifier, ~entityId, ~shouldSaveHistory=false),
          ~shouldRollbackOnReorg,
        )
      | {previousEntity: None, entityType: MultiTokenERC20_TokensBurned, entityId} =>
        inMemStore.multiTokenERC20_TokensBurned->InMemoryTable.Entity.set(
          Delete->Types.mkEntityUpdate(~eventIdentifier=rollBackEventIdentifier, ~entityId, ~shouldSaveHistory=false),
          ~shouldRollbackOnReorg,
        )
      | {previousEntity: None, entityType: MultiTokenERC20_TokensMinted, entityId} =>
        inMemStore.multiTokenERC20_TokensMinted->InMemoryTable.Entity.set(
          Delete->Types.mkEntityUpdate(~eventIdentifier=rollBackEventIdentifier, ~entityId, ~shouldSaveHistory=false),
          ~shouldRollbackOnReorg,
        )
